<div wire:keydown.esc.window="resetData()" class="">
    ...
    @if($user->hasPermission("insurances-create") || $user->hasPermission("insurances-update"))
            ...
            <form class="w-full" wire:submit="save()">
                <div class="flex flex-wrap -mx-3">
                    <div class="w-full md:w-1/4 px-3 mb-6 md:mb-0">
                        ...
                        <input autocomplete="off" @disabled(!empty($currentInsurance)) required
                               wire:model="insuranceName"
                    ...
                    <div class="w-full md:w-1/6 px-3">
                        ...
                        <input autocomplete="off" required
                               @disabled(!empty($currentInsurance)) wire:model="companyEndurance"
                    ...
                    <div class="w-full md:w-1/6 px-3">
                        ...
                        <input autocomplete="off" required
                               @disabled(!empty($currentInsurance)) wire:model="patientEndurance"
                               class="appearance-none text-center block w-full text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                    ...
                    <div class="w-full md:w-1/4 px-3">
                        ...
                        <input autocomplete="off" required
                               @disabled(!empty($currentInsurance)) wire:model="contractDate"
                               class="appearance-none text-center block w-full text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
    ...
    @if(empty($currentInsurance) && $user->hasPermission("insurances-read"))
                ...
                <table class="table-fixed w-full">
                    ...
                    @foreach($insurances as $insurance)
                        <tr class="border-b-2">
                            ...
                            <td>
                                @if($user->hasPermission("insurances-update"))
                                    <button class="bg-cyan-400 p-2 rounded text-xs text-white"
                                            wire:click="edit({{$insurance}})"><i class="fa fa-pen"></i></button>
                                @endif
                                @if($user->hasPermission("insurances-delete"))
                                    <button class="bg-red-400 p-2 rounded text-xs text-white"
                                            wire:click="deleteMessage({{$insurance->id}})"><i class="fa fa-trash"></i>
                                    </button>
                                @endif
                                @if($user->hasPermission("insurances-read"))
                                    <button class="bg-yellow-400 p-2 rounded text-xs text-white"
                                            wire:click="chooseInsurance({{$insurance}})"><i class="fa fa-eye"></i>
                                    </button>
                                @endif
                            </td>
                        </tr>
                ...
                <form class="w-full" wire:submit="saveDebt()">
                    <div class="flex flex-wrap -mx-3">
                        ...
                        <div class="w-full md:w-1/6 px-3">
                            ...
                            <input autocomplete="off" wire:model.live="amount"
                                   class="appearance-none text-center block w-full text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                        ...
                        <div class="w-full md:w-1/6 px-3">
                            ...
                            <input autocomplete="off" wire:model="paid_date"
                                   class="appearance-none text-center block w-full text-gray-700 border border-gray-200 rounded py-3 px-4 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                                   id="paid_date" required type="date">
                        </div>
                            ...
                            @foreach($debts as $debt)
                                <tr class="border-b-2">
                                    ...
                                    <td>
                                        ...
                                        @if($user->hasPermission("insurances-delete"))
                                            <button class="bg-red-400 p-2 rounded text-xs text-white"
                                                    wire:click="deleteDebtMessage({{$debt->id}})"><i
                                                    class="fa fa-trash"></i>
                                            </button>
                                        @endif
